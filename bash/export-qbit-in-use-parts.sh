...
get_qbittorrent_parts_files() {
    ...
    try curl -s --cookie "cookiefile""cookie_file" "url/api/v2/torrents/info" \
        | jq -r '.[] | [.hash, .save_path] | @tsv' \
        | while IFS=$'\t' read -r hash save_path; do
            parts_file="savepath/.{save_path}/.{hash}.parts"
...
try > "OUTPUTDIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_QB_PARTS"  # clear the output file
...
while IFS=" " read -r url user pass; do
    ...
    try get_qbittorrent_parts_files "url""url" "cookie_file" \
        >> "OUTPUTDIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_QB_PARTS"
done < <(try grep -v "^#\|^""" "QB_INSTANCES_FILE")

if $DEBUG; then
    NUMBEROFFILES=(wc−l<"(wc -l < "OUTPUT_DIRECTORY"/"$OUTPUT_FILENAME_QB_PARTS")
    echo "Found $NUMBEROFFILES in-use .parts files:"
    if [ "$NUMBEROFFILES" -gt 0 ]; then
        echo "3"; sleep 1
        echo "2"; sleep 1
        echo "1"; sleep 1
        try sort -u "OUTPUTDIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_QB_PARTS" -o "OUTPUTDIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_QB_PARTS"
        cat "OUTPUTDIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_QB_PARTS" | more
    fi
fi
