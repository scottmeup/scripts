...
if $FREE_SPACE_USE_PERCENTAGE; then
    FREE_SPACE=$(( 100 - `df -P "$DOWNLOADS_DIRECTORY" | tail -1 | awk '{print $5}' | cut -f1 -d%` ))   # Caclulate free space as %
...
#sort -u "OUTPUT_DIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_DOWNLOADS" -o "OUTPUT_DIRECTORY"/"OUTPUT_DIRECTORY"/"OUTPUT_FILENAME_DOWNLOADS"
